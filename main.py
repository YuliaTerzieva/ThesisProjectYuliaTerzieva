...
obsr_frame_orient = math.radians(- (F_true - H_true) - A_ocr * math.sin(abs(H_true)))
...
cont_likelihood = np.zeros(360)
for i in range(0, 4):
    vonmisesX = obsr_frame_orient + phi[i] - np.radians(head_space_orients)
    cont_likelihood += vonmises.pdf(vonmisesX, kappa[i])

cont_likelihood = np.degrees(cont_likelihood)
